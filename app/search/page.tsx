...
export default function SearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/scrape?search=${encodeURIComponent(searchQuery)}`);
      const data = await response.json();

      if (response.ok && Array.isArray(data) && data.length > 0) {
        // Topik ditemukan di database
        setTopics(data);
      } else if (response.status === 201 && Array.isArray(data) && data.length > 0) {
        // Topik tidak ada → scraping berhasil → redirect ke halaman topik
        const newTopic = data[0];
        router.push(`/topic/${newTopic.idtopic}`);
      } else {
        alert("Topik tidak ditemukan dan gagal melakukan scraping.");
      }
    } catch (error) {
  ...
  const isSearching = searchQuery.trim() !== "" && topics.length > 0;
  ...
  const calculateSimilarity = (topic: Topic, query: string): number => {
    ...
    searchTerms.forEach(term => {
      if (titleLower.includes(term)) score += term.length > 3 ? 15 : 10;
      if (descLower.includes(term)) score += term.length > 3 ? 8 : 5;
      ...
      descWords.forEach(word => {
        ...
        else if (word.includes(term) && term.length > 2) score += 2;
        else if (term.includes(word) && word.length > 2) score += 1;
      });
    });
    ...
    >
      ...
      <div
        className={`w-full transition-all duration-500 ease-in-out ${
          isSearching ? "pt-10 pb-0 px-4" : "flex-grow flex items-center justify-center px-4"
        }`}
      >
